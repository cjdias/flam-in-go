...
func Test_JsonLogSerializer(t *testing.T) {
	config := flam.Bag{}
	...
	scenarios := []struct {
	...
	}{
		{
			...
			message:   "message",
			ctx:       flam.Bag{},
			expected:  `{"level":"INFO","message":"message","time":"2025-10-28T11:31:00.000+0000"}` + "\n",
		},
		{
			...
			expected:  `{"ctx":"value","level":"INFO","message":"message","time":"2025-10-28T11:31:00.000+0000"}` + "\n",
		},
	}
	...
	for _, scenario := range scenarios {
		t.Run(scenario.name, func(t *testing.T) {
			assert.NoError(t, app.Container().Invoke(func(factory flam.LogSerializerFactory) {
				...
				result := serializer.Serialize(scenario.timestamp, scenario.level, scenario.message, scenario.ctx)
