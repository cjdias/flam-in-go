...
type miniRedisConnectionCreator struct{}

func newMiniRedisConnectionCreator() RedisConnectionCreator {
	return &miniRedisConnectionCreator{}
}

func (miniRedisConnectionCreator) Accept(
	config Bag,
) bool {
	return config.String("driver") == RedisConnectionDriverMini
}

func (c miniRedisConnectionCreator) Create(
	config Bag,
) (RedisConnection, error) {
	host := config.String("host")

	if host == "" {
		return nil, newErrInvalidResourceConfig("miniRedisConnection", "host", config)
	}
